...
pkcs8'encpri :: Password -> 
  ( (SizeT, Salt, Key) -> BS.ByteString
  , BS.ByteString -> Maybe (SizeT, Salt, Key)
  )
...
pkcs8'wrap ::
  ( (SizeT, Salt, EKey) -> BS.ByteString
  , BS.ByteString -> Maybe (SizeT, Salt, EKey)
  )
...
pkcs8'enc'asn1 ::
  ( (SizeT, Salt, EKey) -> [ASN1.ASN1]
  , [ASN1.ASN1] -> Maybe (SizeT, Salt, EKey)
  )
...
type Theta = BS.ByteString -- 32 byte
type Key = BS.ByteString -- 32 .. 64 byte
type Header = BS.ByteString -- 16 byte
type EKey = BS.ByteString -- sizeof Key + sizeof Header
